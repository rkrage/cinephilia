class UsersController < ApplicationController
  ...
  def user_suggestions
    ...
    User.all.each do |temp_user|
      ...
      if temp_user != @user and (temp_g1 == @first or temp_g1 == @second or temp_g2 == @first or temp_g2 == @second)
        @users << temp_user
      end
    end
    @users = (@users - @user.following).sort {|a,b| b.followers.length <=> a.followers.length}
